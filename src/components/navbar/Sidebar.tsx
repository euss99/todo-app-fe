...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      {isOpen && (
        <div
          className="fixed inset-0 bg-gray-600/30 backdrop-blur-sm transition-opacity lg:hidden z-40"
          onClick={onClose}
        />
      )}

      <div
        className={`fixed inset-y-0 left-0 z-50 w-64 md:w-1/2 lg:w-64 bg-white dark:bg-gray-800 transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 ${transformClasses}`}
      >
